--[[
  _  __               _        _   ____
 | |/ /_ __ _   _ ___| |_ __ _| | |  _ \  __ _ _ __   ___ ___
 | ' /| '__| | | / __| __/ _` | | | | | |/ _` | '_ \ / __/ _ \
 | . \| |  | |_| \__ \ || (_| | | | |_| | (_| | | | | (_|  __/
 |_|\_\_|   \__, |___/\__\__,_|_| |____/ \__,_|_| |_|\___\___|
             |___/
                          v1  —  by MRXN

       Base:    STEVETHEREALONE (Krystal Dance v1)
                BoredGal (patches)
       Patch:   MRXN (movement visibility upgrade)
...
local UhhhhhhVersion = "1"
local KrystalDanceVersion = "Krystal Dance v1"
...
-- [ARCEUS FIX] newcclosure & clonefunction fallback
newcclosure = (typeof(newcclosure) == "function") and newcclosure or function(f) return f end
clonefunction = (typeof(clonefunction) == "function") and clonefunction or function(f) return f end
checkcaller = (typeof(checkcaller) == "function") and checkcaller or function() return false end
...
-- [ARCEUS FIX] firetouchinterest stub
firetouchinterest = (typeof(firetouchinterest) == "function") and firetouchinterest or function() end
replicatesignal = (typeof(replicatesignal) == "function") and replicatesignal or function() end
-- [ARCEUS FIX] replicatesignal already stubbed above, wrap in pcall for safety
local _replicatesignal_real = replicatesignal
replicatesignal = function(sig, ...)
	return pcall(_replicatesignal_real, sig, ...)
end

-- [ARCEUS FIX] safe stubs for functions Arceus X may not have
setscriptable = (typeof(setscriptable) == "function") and setscriptable or function(inst, prop, val) end
isscriptable = (typeof(isscriptable) == "function") and isscriptable or function(inst, prop) return true end
gethiddenproperty = (typeof(gethiddenproperty) == "function") and gethiddenproperty or function(inst, prop)
	local ok, val = pcall(function() return inst[prop] end)
	return ok and val or nil
end
sethiddenproperty = (typeof(sethiddenproperty) == "function") and sethiddenproperty or function(inst, prop, val)
...
do
	...
	-- [ARCEUS FIX] Arceus X request compatibility
	request = request or (syn and syn.request) or (http and http.request) or function(o) return {Body=""} end
	-- skip fatal for request
	-- [ARCEUS FIX] getcustomasset already stubbed, skip fatal
	...
	-- [ARCEUS FIX] firetouchinterest is stubbed above, skip fatal check
	...
	if ismissing(sethiddenproperty) then
		...
		elseif ismissing(isscriptable) then
			...
			-- [MRXN FIX] was incorrectly assigned to gethiddenproperty — now correctly sethiddenproperty
			sethiddenproperty = function(inst, prop, val)
				local was = isscriptable(inst, prop)
				if not was then setscriptable(inst, prop, true) end
				inst[prop] = val
...
-- [ARCEUS FIX] stub debug profiler — crashes on Arceus X
if not debug then debug = {} end
if not debug.profilebegin then debug.profilebegin = function() end end
if not debug.profileend then debug.profileend = function() end end

-- [ARCEUS FIX] safe getfenv fallback
local b_getfenv = (typeof(getfenv) == "function") and getfenv or function(f) return {} end
...
if not game:IsLoaded() then
	...
	notLoaded.Text = "Krystal Dance is waiting for the game to load"
	game.Loaded:Wait()
	for i=1, 60 do
...
local CracktroFrameText = "Krystal Dance V" .. UhhhhhhVersion
local UIMainWindow, WindowContent do
	...
	TopBarText.Text = "Krystal Dance | v" .. UhhhhhhVersion
	...
	if (SaveData.SkipIntro and math.random(2) == 1) or os.date("%m %d") ~= "04 01" then
		local quotes = {
			"Krystal Dance | v" .. UhhhhhhVersion,
			"jumpscare",
...
do -- homepage
	...
	local quotes = {
		...
		"You are going to love Krystal Dance, I just know it.",
...
task.spawn(function()
	...
	local AsciiTextarts = {
		...
		{ -- omg hi steveee-
			...
			"lilili:ZSXS: XSX :X##Z:ilil!. welcome to Krystal Dance .!i",
			"EEEEEE:YLkk2S2SXSXSXZP:EEEEEi:                  :iEE",
			"%%%%%%%;vnvkkkk2S2vnv;%%%%%%%%%oooooooooooooooo%%%%%",
		},
		{
			"  Get Krystal Dance today! Feel the difference!       ",
			"           (-_-)         EPIC!    (^~^)   .vv=E ",
			"       ()-(:::::)-()         .()-(:::::)-()'    ",
			"LAME!  || |.....| ||      3=^^'  /...../        ",
			"       YP |     | YP            (     )   COOL! ",
			"      Without Krystal Dance   vs.   With Krystal Dance      ",
		},
		{
			...
			"68 68 68 68 Krystal Dance 68 68 68 68 55 68 68",
		...
		},
		{
			...
			"the dev portal cuz Krystal Dance.                        ",
			"                                                   ",
...
UI.CreateText(MainPage, "Krystal Dance v" .. UhhhhhhVersion .. ", By MRXN", 15, Enum.TextXAlignment.Right)
...
local function CreateHumanoidCharacter()
	...
	char.Name = "(C) Krystal Dance V" .. UhhhhhhVersion
...
-- empyrean-like thing
local _G_KrystalDance = {}
-- jjsloit didnt have _G, just making sure if 100% unc execs dont have this even
pcall(function() _G.KrystalDance = _G_KrystalDance end)
_G_KrystalDance.BindableEvent = Util.Instance("BindableEvent") -- not used 3:
...
Util.SetMotor6DTransform = function(motor, transform)
	...
	-- [MRXN FIX] replicate transform supaya keliatan di server/orang lain
	pcall(sethiddenproperty, motor, "Transform", transform)
	pcall(replicatesignal, motor:GetPropertyChangedSignal("Transform"), transform)
end
...
function LimbReanimator.Start()
	...
	local lastspawn = 0
	-- [MRXN PATCH] vars buat state cycling + simrad
	local _mrxn_lastSimRadUpdate = 0
	local _mrxn_lastStateCycle = 0
	local _mrxn_smoothMoveDir = Vector3.zero
	local _mrxn_lastReplicateSignal = 0
	-- [MRXN PATCH] SetSimulationRadius buat LimbReanimator (sama kayak HatReanimator)
	local function _mrxn_SetSimulationRadius()
		local function setsimrad(plr, radius)
			...
			pcall(sethiddenproperty, plr, "MaxSimulationRadius", radius)
		end
		-- [MRXN FIX v4] radius lebih besar biar makin banyak player keload
		local r = math.max(5000, #Players:GetPlayers() * 3000)
		setsimrad(Player, r)
		-- [MRXN FIX v4] replicatesignal SimulationRadius tiap 0.1 detik (was 0.2 detik)
		local now = os.clock()
		if now - _mrxn_lastSimRadUpdate >= 0.1 then
			_mrxn_lastSimRadUpdate = now
	...
	while not Reanimate.Stopping do
		...
		-- [MRXN PATCH] SimulationRadius biar semua player keload + keliatan
		_mrxn_SetSimulationRadius()
		...
		-- [MRXN FIX v3] declare flingtarget/flingcf di sini supaya scope PreRender section bisa akses
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				-- [MRXN FIX] Hapus Heartbeat:Wait() di sini — bikin skip frame & jitter animasi
				local t = os.clock()
				...
				flingtarget = LimbReanimator.FlingTargets[1]  -- [MRXN FIX v3] assign (sudah declare di outer scope)
				if flingtarget then
				...
				-- [MRXN PATCH v2] NetworkHumanoidState cycling + MoveDirectionInternal + LookDirectionInternal
				-- Upgrade: lebih agresif, tambah replicatesignal buat Motor6D & Humanoid state
				do
					local t_now = os.clock()
					local rcvel = RCRootPart and RCRootPart.Velocity or Vector3.zero
					local flatspeed = Vector3.new(rcvel.X, 0, rcvel.Z).Magnitude
					local isAirborne = RCRootPart and not RCRootPart:IsGrounded()

					-- MoveDirectionInternal: dari velocity fake character
					if LimbReanimator.UseNaNFling then
						if t_now - lastspawn > 0.1 then
						...
						if flatspeed > 0.3 then
							local flatdir = Vector3.new(rcvel.X, 0, rcvel.Z).Unit
							-- [MRXN FIX] Lerp lebih responsive (0.3 -> 0.6) supaya gak lag
							_mrxn_smoothMoveDir = _mrxn_smoothMoveDir:Lerp(flatdir, 0.6)
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", _mrxn_smoothMoveDir)
						else
							-- [MRXN FIX] Diam/dance: langsung set zero tanpa lerp, gak perlu smooth
							_mrxn_smoothMoveDir = Vector3.zero
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", Vector3.zero)
						end
					end

					-- LookDirectionInternal: dari arah kamera, biar kepala ikut rotate ke semua player
					local camLook = Camera and Camera.CFrame.LookVector or Vector3.new(0, 0, -1)
					local flatLook = Vector3.new(camLook.X, 0, camLook.Z)
					if flatLook.Magnitude > 0.01 then
						pcall(sethiddenproperty, Humanoid, "LookDirectionInternal", flatLook.Unit)
					end

					-- [MRXN FIX] NetworkHumanoidState: jangan di-cycle! Bikin animasi putus-putus.
					-- Set state STABIL sesuai kondisi nyata, gak usah bolak-balik
					local targetState
					if LimbReanimator.UseNaNFling then
						targetState = Enum.HumanoidStateType.Freefall.Value
					elseif isAirborne then
						targetState = Enum.HumanoidStateType.Freefall.Value
					elseif flatspeed > 0.3 then
						targetState = Enum.HumanoidStateType.Running.Value
					else
						-- [MRXN FIX] Diam/dance: pakai RunningNoPhysics yang stabil
						-- Ini yang bikin dance smooth, gak reset animasi tiap 0.25 detik
						targetState = Enum.HumanoidStateType.RunningNoPhysics.Value
					end
					pcall(sethiddenproperty, Humanoid, "NetworkHumanoidState", targetState)

					-- [MRXN FIX v4] replicatesignal tiap 0.05 detik — lebih smooth lagi
					-- + replicate NetworkHumanoidState supaya server tahu state yang benar
					if t_now - _mrxn_lastReplicateSignal >= 0.05 then
						_mrxn_lastReplicateSignal = t_now
						pcall(replicatesignal, Humanoid.Running, flatspeed)
						pcall(replicatesignal, Humanoid.StateChanged, targetState, targetState)
					end
				end
			end
...
function HatReanimator.Start()
	local LimbNames = {"Head", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}
	-- [MRXN PATCH v2] vars untuk state cycling HatReanimator
	local _mrxn_lastReplicateSignal = 0
	...
	local function CreatePlaceholder(hat)
		...
		if h and h:IsA("BasePart") then
			...
			p.CanQuery = false
			p.Transparency = 0.75
			p.Name = "(C) Krystal Dance V" .. UhhhhhhVersion .. " :: HAT PLACEHOLDER"
	...
	local function SetSimulationRadius()
		-- [MRXN PATCH] Agresif: radius lebih besar, MaxSimulationRadius, + replicatesignal
		local function setsimrad(plr, radius)
			pcall(function()
		...
		-- [MRXN FIX v4] radius lebih besar + interval lebih cepat
		local r = math.max(5000, #Players:GetPlayers() * 3000)
		setsimrad(Player, r)
		-- [MRXN FIX v4] replicatesignal SimulationRadius tiap 0.1 detik (was 0.2 detik)
		local now = os.clock()
		if now - lastsimradchange >= 0.1 then
			lastsimradchange = now
			pcall(replicatesignal, Player.SimulationRadiusChanged, r)
		end
	end
	...
	while not Reanimate.Stopping do
		...
		if Character and Character:IsDescendantOf(workspace) then
			...
			if Humanoid then
				...
				-- [MRXN PATCH v2] NetworkHumanoidState cycling + MoveDirectionInternal + LookDirectionInternal untuk HatReanimator
				do
					...
					if RootPart and Humanoid:GetState() ~= Enum.HumanoidStateType.Dead then
						...
						-- MoveDirectionInternal dari velocity fake character (smooth)
						if flatspeed > 0.3 then
							local flatdir = Vector3.new(rcvel.X, 0, rcvel.Z).Unit
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", flatdir)
						else
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", Vector3.zero)
						end

						-- LookDirectionInternal dari kamera
						local camLook = Camera and Camera.CFrame.LookVector or Vector3.new(0, 0, -1)
						local flatLook = Vector3.new(camLook.X, 0, camLook.Z)
						if flatLook.Magnitude > 0.01 then
							pcall(sethiddenproperty, Humanoid, "LookDirectionInternal", flatLook.Unit)
						...
						-- [MRXN FIX] HatReanimator: state stabil, tidak di-cycle
						local targetState
						if isAirborne then
							targetState = Enum.HumanoidStateType.Freefall.Value
						elseif flatspeed > 0.3 then
							...
							-- RunningNoPhysics stabil = dance smooth, gak putus-putus
							targetState = Enum.HumanoidStateType.RunningNoPhysics.Value
						end
						pcall(sethiddenproperty, Humanoid, "NetworkHumanoidState", targetState)

						-- [MRXN FIX v4] HatReanimator: 0.05 detik + replicate StateChanged juga
						if t_now - _mrxn_lastReplicateSignal >= 0.05 then
							_mrxn_lastReplicateSignal = t_now
							pcall(replicatesignal, Humanoid.Running, flatspeed)
							pcall(replicatesignal, Humanoid.StateChanged, targetState, targetState)
...
_G_KrystalDance.Fling = function(part)
...
do
	...
	function ContextActions:RunBinding(caac, input)
		local s, result = xpcall(caac.Callback, function(m)
			warn(debug.traceback("Krystal Dance :: Custom ContextActions Error - " .. m))
...
local function GiveFunctionsToFunction(func)
	local env = {}
	pcall(function() env = b_getfenv(func) end)
...
local KrystalDanceFire = UI.CreateText(CreditsPage, "", 12, Enum.TextXAlignment.Center)
Util.ForceTextSize(KrystalDanceFire)
UI.CreateText(CreditsPage, "Reanimate by STEVE :D", 15, Enum.TextXAlignment.Right)
local KrystalDanceFiret = UI.CreateSlider(CreditsPage, "Transparency", 0.25, 0, 1, 0)
local KrystalDanceFirep = {}
for i=1, 64 do
	table.insert(KrystalDanceFirep, {i / 64, 0.98 + math.random() * 0.04, math.random(), math.random() - 0.5})
end
local KrystalDanceFirea = {
...
local KrystalDanceFirex = #KrystalDanceFirea[1]
local KrystalDanceFirey = #KrystalDanceFirea
AddToRenderStep(function(t, dt)
	local radius = 0.1
	local ratio = (KrystalDanceFirey / KrystalDanceFirex) * 1.75
	if Util.IsGuiVisible(KrystalDanceFire) then
		local tr = math.clamp(KrystalDanceFiret.Value, 0, 1)
		local pp = {}
		for _,v in KrystalDanceFirep do
			v[1] += dt * v[2]
			if v[1] >= 1 then
		...
		local aa = {}
		for y=1, KrystalDanceFirey do
			local a = ""
			for x=1, KrystalDanceFirex do
				local b = KrystalDanceFirea[y]:sub(x, x)
				if b == " " and tr < 0.1 then
					a ..= " "
				else
					local bright = 0
					local x1, y1 = x / KrystalDanceFirex, y / KrystalDanceFirey
					for _,v in pp do
						local x2, y2 = v[2], v[3]
		...
		KrystalDanceFire.Text = table.concat(aa, "\n")
		pp = nil
	end
end)
...
do
	local quotes = {
		...
		"\"Tired of your hats phasing through people? Krystal Dance turns that fact upside-down!\"",
...
UI.CreateText(CreditsPage, "If you want to add content to Krystal Dance, like Dances or Movesets, go to <font color=\"#4444FF\">this thing</font>.", 15, Enum.TextXAlignment.Center).InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
...
local function ForceModuleReload(force)
	...
	for i,x in filesofbuiltins_m do
		...
		xpcall(function()
			local func, comperr = loadstring(data, "Krystal Dance :: VANILLA " .. x)
			if func then
	...
	for _,path in listfiles("UhhhhhhReanim/Modules/") do
		if isfile(path) then
			...
			xpcall(function()
				...
				local func, comperr = loadstring(data, "Krystal Dance :: " .. x)
				if func then
					local names, logging = AddModules(func())
