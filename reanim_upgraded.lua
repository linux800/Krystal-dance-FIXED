-- [UPGRADE] reanim_fixed.lua compatibility/stability pass
--[[
  _  __               _        _   ____
 ...
 |_|\_\_|   \__, |___/\__\__,_|_| |____/ \__,_|_| |_|\___\___|
       ...
       Base:    STEVETHEREALONE (Uhhhhhh Reanimate)
                BoredGal (patches)
...
-- Safe protected-call wrappers. Keep the original globals before shadowing them.
local _native_pcall = pcall
local _native_xpcall = xpcall
local pcall = function(func, ...)
	return _native_pcall(function(...)
		return func(...)
	end, ...)
end
local xpcall = function(func, ...)
	return _native_xpcall(function(...)
		return func(...)
	end, ...)
end
...
-- [UPGRADE] compatibility helpers
-- These helpers only make object access safer; they do not bypass server rules.
local function _safeFind(parent, name, className)
...
local function _safeConnect(signal, callback)
	...
	return signal:Connect(function(...)
		...
		if not ok then
			warn("Krystal Dance :: callback error:", err)
		end
	end)
end

local function _safeDestroy(obj)
...
UI.CreateText(MainPage, `Krystal Dance {UhhhhhhVersion}, By MRXN`, 15, Enum.TextXAlignment.Right)
...
function LimbReanimator.Start()
	...
	local function _mrxn_SetSimulationRadius()
		...
		local r = math.max(2000, #Players:GetPlayers() * 2000)
		setsimrad(Player, r)
		-- [UPGRADE] SimulationRadius refresh throttled to reduce client churn
		local now = os.clock()
		if now - _mrxn_lastSimRadUpdate >= 0.5 then
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				do
					...
					-- [UPGRADE] Throttle Running signal to reduce event spam
					-- 0.5 detik terlalu jarang: orang lain lihat animasi freeze/kedet tiap 0.5 detik
					if t_now - _mrxn_lastReplicateSignal >= 1 / 30 then
...
function HatReanimator.Start()
	...
	local function SetSimulationRadius()
		...
		-- radius = makin banyak player makin besar, minimal 2000 biar semua keliatan
		local r = math.max(2000, #Players:GetPlayers() * 2000)
		setsimrad(Player, r)
		-- [UPGRADE] SimulationRadius refresh throttled to reduce client churn
		local now = os.clock()
		if now - lastsimradchange >= 0.5 then
			...
			if Humanoid then
				...
				do
					...
					if RootPart and Humanoid:GetState() ~= Enum.HumanoidStateType.Dead then
						...
						-- [UPGRADE] Throttle HatReanimator Running signal
						if t_now - _mrxn_lastReplicateSignal >= 1 / 30 then
							_mrxn_lastReplicateSignal = t_now
							pcall(replicatesignal, Humanoid.Running, flatspeed)
						end
