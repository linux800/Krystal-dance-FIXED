...
Reanimate.CreateCharacter = function(InitCFrame)
	...
	Util.LinkDestroyI2C(RC, RunService.PostSimulation:Connect(function(dt)
		...
		local void = true
		local rayHits = 0
		-- [FIX] Kurangi raycast dari 8 ke 4, ray lebih pendek (2048), void hanya true kalau SEMUA ray miss
		for i=1, 4 do
			local off = CFrame.Angles(0, (i / 2) * math.pi, 0):VectorToWorldSpace(Vector3.new(0, 0, -0.5))
			local r = workspace:Raycast(RCRootPart.Position + off, Vector3.new(0, -2048, 0), RCP)
			if r then
				rayHits += 1
				void = false
		...
		-- void hanya true kalau benar-benar tidak ada ground sama sekali
		if rayHits == 0 then
			void = true
		end
		if rayHits < 4 then
...
function LimbReanimator.Start()
	...
	local function _mrxn_SetSimulationRadius()
		...
		setsimrad(Player, r)
		local now = os.clock()
		if now - _mrxn_lastSimRadUpdate >= 1 then
				...
				do
					...
					if LimbReanimator.UseNaNFling then
						...
						if flatspeed > 0.3 then
							...
							-- smooth supaya gak glitchy
							_mrxn_smoothMoveDir = _mrxn_smoothMoveDir:Lerp(flatdir, 0.3)
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", _mrxn_smoothMoveDir)
						else
							_mrxn_smoothMoveDir = _mrxn_smoothMoveDir:Lerp(Vector3.zero, 0.4)
							pcall(sethiddenproperty, Humanoid, "MoveDirectionInternal", _mrxn_smoothMoveDir)
						end
					end
					...
					-- NetworkHumanoidState cycling agresif:
					-- Cycle lebih cepet (4x per detik) supaya server gak ignore
					local cyclePhase = math.floor(t_now * 4) % 4
					local targetState
					if LimbReanimator.UseNaNFling then
					...
					elseif flatspeed > 0.3 then
						...
						-- diam: cycle 3 state supaya gak statis
						-- Running → RunningNoPhysics → Running → RunningNoPhysics
						if cyclePhase % 2 == 0 then
							targetState = Enum.HumanoidStateType.Running.Value
						else
					...
					-- [FIX] replicatesignal hanya Humanoid.Running saja, tidak spam Motor6D tiap frame
					-- Motor6D replicate otomatis via physics engine, tidak perlu manual
					if t_now - _mrxn_lastReplicateSignal >= 0.5 then
...
function HatReanimator.Start()
	local LimbNames = {"Head", "Torso", "Left Arm", "Right Arm", "Left Leg", "Right Leg"}
	-- [FIX] vars untuk state cycling HatReanimator
	local _mrxn_lastReplicateSignal = 0
	...
	local function SetSimulationRadius()
		...
		local r = math.max(2000, #Players:GetPlayers() * 2000)
		setsimrad(Player, r)
		-- replicatesignal tiap ~1 detik buat paksain server tau radius baru
		local now = os.clock()
		if now - lastsimradchange >= 1 then
			...
			if Humanoid then
				...
				do
					...
					if RootPart and Humanoid:GetState() ~= Enum.HumanoidStateType.Dead then
						...
						-- NetworkHumanoidState cycling agresif (4x per detik)
						local cyclePhase = math.floor(t_now * 4) % 4
						local targetState
						if isAirborne then
							targetState = Enum.HumanoidStateType.Freefall.Value
						...
						-- [MRXN PATCH v2] replicatesignal tiap 0.5 detik buat HatReanimator juga
						if t_now - _mrxn_lastReplicateSignal >= 0.5 then
							_mrxn_lastReplicateSignal = t_now
							pcall(replicatesignal, Humanoid.Running, flatspeed)
						end
