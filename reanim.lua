...
function LimbReanimator.Start()
	...
	local lastspawn = 0
	-- [MRXN PATCH] vars buat state cycling
	...
	while not Reanimate.Stopping do
		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				-- [MRXN PATCH] NetworkHumanoidState cycling + MoveDirectionInternal + LookDirectionInternal
				do
					...
					-- MoveDirectionInternal: selalu set dari velocity fake character, bukan cuma pas NaNFling
					if LimbReanimator.UseNaNFling then
						...
						if flatspeed > 0.5 then
							-- lagi jalan: kirim arah gerak ke server biar animasi walk keliatan
					...
					-- LookDirectionInternal: dari arah kamera, biar kepala ikut rotate
					...
					-- NetworkHumanoidState cycling: gak statis biar gak di-ignore game
					local targetState
					if LimbReanimator.UseNaNFling then
					...
					elseif flatspeed > 0.5 then
						-- jalan: pakai Running, state paling dikenali semua game
						targetState = Enum.HumanoidStateType.Running.Value
					else
						-- diam tapi tetap cycle antara Running/RunningNoPhysics supaya gak ke-detect statis
						if math.floor(t_now * 2) % 2 == 0 then
							targetState = Enum.HumanoidStateType.Running.Value
						else
	...
	while not Reanimate.Stopping do
		...
		if Character and Character:IsDescendantOf(workspace) then
			...
			if Humanoid then
				...
				-- [MRXN PATCH] NetworkHumanoidState cycling + MoveDirectionInternal + LookDirectionInternal untuk HatReanimator
				do
