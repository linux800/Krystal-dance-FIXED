		...
		if ReanimCharacter then
			...
			if Character and Humanoid and RootPart then
				...
				do
					...
					-- [MRXN PATCH v2] replicatesignal tiap ~0.5 detik buat paksa server tau state baru
					-- Ini yang bikin dance keliatan orang lain!
					if t_now - _mrxn_lastReplicateSignal >= 0.5 then
						...
						-- Paksa replicate motor6d transforms ke semua client
						for _, map in LimbMapping do
							if map.Reference then
								local v = map.Reference
								pcall(replicatesignal, v.Changed, v.Transform)
							end
						end
					end
				end
